...
class DatasetCOCOALL(Dataset):
    ...
    def __len__(self):
        return 32000 if self.split == 'trn' else 1000
    ...
    def __getitem__(self, idx):
        ...
        query_img, query_mask, query_mask_orig, support_imgs, support_masks, query_name, support_names, class_sample, org_qry_imsize = self.load_frame()

        query_img = self.transform(query_img)
        query_mask = query_mask.float()
        query_mask_orig = query_mask_orig.float()
        if not self.use_original_imgsize:
            query_mask = F.interpolate(query_mask.unsqueeze(0).unsqueeze(0).float(), query_img.size()[-2:], mode='nearest').squeeze()
            query_mask_orig = F.interpolate(query_mask_orig.unsqueeze(0).unsqueeze(0).float(), query_img.size()[-2:], mode='nearest').squeeze()
        ...
        batch = {'query_img': query_img,
                 ...
                 'query_mask_orig': query_mask_orig,
    ...
    def load_frame(self):
        ...
        query_mask_orig = query_mask.clone()
        ...
        return query_img, query_mask, query_mask_orig, support_imgs, support_masks, query_name, support_names, class_sample, org_qry_imsize